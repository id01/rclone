...
func gzipExtraify(in io.Reader, out io.Writer) error {
	...
	for {
		...
		_, err = out.Write(currGzipData)
		if err != nil {
			return err
		}
	}
	_, err := out.Write(append(gzipHeaderData, []byte{0x04, 0x00}...)) // 4 bytes
	if err != nil {
		return err
	}
	_, err = out.Write(append(uint32ToBytes(totalLength), gzipContentAndFooter...))
	return err
}
...
func (c *Compression) compressBlock(in []byte, out io.Writer) (compressedSize uint32, uncompressedSize int64, err error) {
	...
	case XzInGz:
		return c.compressBlockExecGz(in, out, c.BinPath, []string{"-c"})
	case XzInGzMin:
		return c.compressBlockExecGz(in, out, c.BinPath, []string{"-c1"})
	case LZ4:
...
func (d *Decompressor) decompressBlock(in io.Reader, out io.Writer) (n int, err error) {
	...
	case XzInGzMin:
		return decompressBlockRangeExecGz(in, out, d.c.BinPath, []string{"-dc1"})
	case XzInGz:
		return decompressBlockRangeExecGz(in, out, d.c.BinPath, []string{"-dc"})
	case LZ4:
...
func (d *Decompressor) initReadingBlockData(c *Compression, in io.ReadSeeker, size int64) error {
	...
	_, err = in.Seek(size-trailingBytes-int64(gzippedBlockDataLen), io.SeekStart)
	if err != nil {
		return err
	}
	gzippedBlockData := make([]byte, gzippedBlockDataLen)
	_, err = in.Read(gzippedBlockData)
	if err != nil {
		return err
	}
	...
	for {
		...
		_, err = gzippedBlockDataRawReader.Read(gzipExtraDataLenBytes)
		if err != nil {
			return err
		}
		...
		_, err = gzippedBlockDataRawReader.Read(gzipExtraData)
		if err != nil {
			return err
		}
